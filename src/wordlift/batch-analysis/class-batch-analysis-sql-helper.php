...
class Batch_Analysis_Sql_Helper {
	 ...
	 * @since 3.14.2
	 */
	public static function get_sql( $fields, $args, $limit = PHP_INT_MAX, $offset = 0 ) {
		global $wpdb;
		...
		*/
		...
		return "SELECT $fields FROM $wpdb->posts p"
		       . " WHERE p.post_type IN ('" . join( "', '", array_map( 'esc_sql', (array) $args['post_type'] ) ) . "')"
		       . "  AND p.post_status = 'publish'"
		       . self::and_include_annotated( $args['include_annotated'] )
		       . self::and_post_date_from( $args['from'] )
		       . self::and_post_date_to( $args['to'] )
		       . self::and_exclude_posts( $args['exclude'] )
		       . ' ORDER BY p.ID ASC'
			   . ' LIMIT ' . esc_js( $offset ) . ', ' . esc_sql( $limit );
