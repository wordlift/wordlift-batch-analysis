...
class Ajax_Adapter {
	...
	function start() {
		...
		$ajax_progress = new Ajax_Progress( $this->action_name );

		// Finally create the task runner and start it.
		$task_runner = new Single_Instance_Task_Runner( $this->task, true, array( $ajax_progress ) );

		try {
			// Start the task runner, 1 item at a time.
			$task_runner->start( $limit, $offset );
		} catch ( Another_Instance_Is_Running_Exception $e ) {
